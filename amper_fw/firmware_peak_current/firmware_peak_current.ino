 ...
 *
 *  Алгоритм:
 *      - Непрерывный сбор отсчётов АЦП: среднее и пик за окно 100 мс.
...
#define RAW_AVG_SAMPLES     100         // Кол-во отсчётов для команды RAW
...
uint32_t adcSum = 0;            // Сумма отсчётов в текущем окне
uint16_t adcCount = 0;          // Количество отсчётов в текущем окне
uint16_t adcPeak = 0;           // Максимальный отсчёт в текущем окне
...
void     sendDataLine(float averageVolts, float peakVolts, float instantaneousVolts);
void     sendPointsList();
...
void loop() {
    ...
    /* ---- 2. Непрерывный сбор АЦП для среднего и пика ---- */
    uint16_t sample = analogRead(SENSOR_PIN);
    adcSum += sample;
    adcCount++;
    if (sample > adcPeak) adcPeak = sample;

    /* ---- 3. Периодическая отправка DATA:... (каждые 100 мс) ---- */
    ...
    if (now - lastDataMs >= DATA_PERIOD_MS) {
        ...
        float averageCounts = (float)adcSum / (float)adcCount;
        float averageVolts = averageCounts * (ADC_REF_VOLTAGE / ADC_MAX_COUNTS);
        float peakVolts = (float)adcPeak * (ADC_REF_VOLTAGE / ADC_MAX_COUNTS);
        float instantaneousVolts = (float)sample * (ADC_REF_VOLTAGE / ADC_MAX_COUNTS);
        sendDataLine(averageVolts, peakVolts, instantaneousVolts);
        adcSum = 0;
        adcCount = 0;
        adcPeak = 0;
    }
...
/*
 *  Считать RAW_AVG_SAMPLES отсчётов АЦП и вернуть среднее напряжение.
 *  Используем плавающее накопление для точности.
 */
float readAverageVolts() {
    long sum = 0;
    for (uint8_t i = 0; i < RAW_AVG_SAMPLES; i++) {
        sum += analogRead(SENSOR_PIN);
    }
    float avgCounts = (float)sum / (float)RAW_AVG_SAMPLES;
...
void sendDataLine(float averageVolts, float peakVolts, float instantaneousVolts) {
    float averageAmps = voltsToAmps(averageVolts);
    float peakAmps = voltsToAmps(peakVolts);
    float instantaneousAmps = voltsToAmps(instantaneousVolts);

    // Запрет отрицательных токов (нет инверсии)
    if (averageAmps < 0.0f) averageAmps = 0.0f;
    if (peakAmps < 0.0f) peakAmps = 0.0f;
    if (instantaneousAmps < 0.0f) instantaneousAmps = 0.0f;

    // Формат: DATA:<average_amps>:<peak_amps>:<instant_amps>:<average_volts>\n
    char buf[80];
    char vStr[12];
    char averageStr[12];
    char peakStr[12];
    char instantaneousStr[12];
    dtostrf(averageAmps, 0, 3, averageStr);
    dtostrf(peakAmps, 0, 3, peakStr);
    dtostrf(instantaneousAmps, 0, 3, instantaneousStr);
    dtostrf(averageVolts, 0, 3, vStr);
    snprintf(buf, sizeof(buf), "DATA:%s:%s:%s:%s", averageStr, peakStr, instantaneousStr, vStr);
    Serial.print(buf);
    Serial.print('\n');
}
